...
JL_DLLEXPORT void *ParseDeclaration(CxxInstance *Cxx,
                                    clang::DeclContext *DCScope) {
  auto *P = Cxx->Parser;
...
JL_DLLEXPORT void ParseParameterList(CxxInstance *Cxx, void **params,
                                     size_t nparams) {
  auto *P = Cxx->Parser;
  auto *S = &Cxx->CI->getSema();
  if (P->getPreprocessor().isIncrementalProcessingEnabled() &&
  ...
  clang::Parser::ParseScope PrototypeScope(
...
JL_DLLEXPORT void EnterSourceFile(CxxInstance *Cxx, char *data, size_t length) {
  ...
  clang::FileID FID = sm.createFileID(
      llvm::MemoryBuffer::getMemBufferCopy(llvm::StringRef(data, length)),
      clang::SrcMgr::C_User, 0, 0, sm.getLocForStartOfFile(sm.getMainFileID()));
  clang::Preprocessor &P = Cxx->Parser->getPreprocessor();
  P.EnterSourceFile(FID, CurDir, sm.getLocForStartOfFile(sm.getMainFileID()));
}

JL_DLLEXPORT void EnterVirtualFile(CxxInstance *Cxx, char *data, size_t length,
                                   char *VirtualPath, size_t PathLength) {
  const clang::DirectoryLookup *CurDir = nullptr;
  clang::FileManager &fm = Cxx->CI->getFileManager();
  clang::SourceManager &sm = Cxx->CI->getSourceManager();
  ...
  clang::FileID FID =
      sm.createFileID(Entry, sm.getLocForStartOfFile(sm.getMainFileID()),
                      clang::SrcMgr::C_User);
  clang::Preprocessor &P = Cxx->Parser->getPreprocessor();
  P.EnterSourceFile(FID, CurDir, sm.getLocForStartOfFile(sm.getMainFileID()));
}
...
JL_DLLEXPORT bool BuildNNS(CxxInstance *Cxx, clang::CXXScopeSpec *spec,
                           const char *Name) {
  clang::Preprocessor &PP = Cxx->CI->getPreprocessor();
  // Get the identifier.
  clang::IdentifierInfo *Id = PP.getIdentifierInfo(Name);
  clang::Sema::NestedNameSpecInfo NNSI(Id, getTrivialSourceLocation(Cxx),
                                       getTrivialSourceLocation(Cxx),
                                       clang::QualType());
  return Cxx->CI->getSema().BuildCXXNestedNameSpecifier(
      nullptr, NNSI, false, *spec, nullptr, false, nullptr);
}

JL_DLLEXPORT void *lookup_name(CxxInstance *Cxx, char *name,
  ...
  clang::LookupResult R(cs, DName, getTrivialSourceLocation(Cxx),
...
JL_DLLEXPORT void *SpecializeClass(CxxInstance *Cxx,
                                   clang::ClassTemplateDecl *tmplt,
                                   void **types, uint64_t *integralValues,
                                   int8_t *integralValuePresent, size_t nargs) {
  ...
  for (size_t i = 0; i < nargs; ++i) {
    if (integralValuePresent[i] == 1) {
      ...
      targs[i] =
          clang::TemplateArgument(clang::QualType::getFromOpaquePtr(types[i]));
    }
    targs[i] = Cxx->CI->getASTContext().getCanonicalTemplateArgument(targs[i]);
  }
  void *InsertPos;
  clang::ClassTemplateSpecializationDecl *ret = tmplt->findSpecialization(
      ArrayRef<clang::TemplateArgument>(targs, nargs), InsertPos);
  if (!ret) {
...
JL_DLLEXPORT void *createNamespace(CxxInstance *Cxx, char *name) {
  ...
  return (void *)clang::NamespaceDecl::Create(
      ...
      Cxx->CI->getASTContext().getTranslationUnitDecl(), false,
      getTrivialSourceLocation(Cxx), getTrivialSourceLocation(Cxx), Id,
      nullptr);
}

// TODO: xx
JL_DLLEXPORT void SetDeclInitializer(CxxInstance *Cxx, clang::VarDecl *D,
                                     LLVMValueRef ConstInt) {
  auto CI = llvm::unwrap<llvm::Constant>(ConstInt);
  ...
  GV->setInitializer(
      ConstantExpr::getBitCast(CI, GV->getType()->getElementType()));
  GV->setConstant(true);
}
...
static Function *CloneFunctionAndAdjust(
    CxxInstance *Cxx, Function *F, FunctionType *FTy, bool ModuleLevelChanges,
    ClonedCodeInfo *CodeInfo, const clang::CodeGen::CGFunctionInfo &FI,
    clang::FunctionDecl *FD, bool specsig, bool firstIsEnv, bool *needsbox,
    void *jl_retty, void **juliatypes, bool newgc) {
  ...
  for (const llvm::GlobalVariable &GV : M->globals()) {
    if (GV.hasPrivateLinkage() && GV.isConstant() && GV.hasInitializer()) {
      llvm::GlobalVariable *GVP = cast<llvm::GlobalVariable>(
          Cxx->shadow->getOrInsertGlobal(GV.getName(), GV.getValueType()));
      // GVP->print(llvm::errs(), false);
      GVP->setInitializer(const_cast<llvm::Constant *>(GV.getInitializer()));
      GVP->setConstant(GV.isConstant());
      GVP->setDSOLocal(false);
      GVP->setLinkage(llvm::GlobalValue::LinkageTypes::LinkOnceODRLinkage);
      GVP->setUnnamedAddr(llvm::GlobalValue::UnnamedAddr::Global);
      GVP->setAttributes(GV.getAttributes());
    }
  }

  for (llvm::Function &FN : M->functions()) {
    Cxx->shadow->getOrInsertFunction(FN.getName(), FN.getFunctionType(), {});
  }

  // Create the new function...
  Function *NewF =
      Function::Create(FTy, F->getLinkage(), F->getName(), Cxx->shadow);
  ...
  if (needsbox) {
    ...
    if (newgc) {
      ...
      // Value *ptls_ptr =
      //     CallInst::Create(PTLSStates, {}, "", &*NewF->front().begin());
      ...
      for (auto *PVD : Args) {
        ...
        if (needsbox[i]) {
          ...
          Value *Allocation = CallInst::Create(
              jl_alloc_obj_func,
              {// {cast<Value>(CastInst::Create(Instruction::BitCast, ptls_ptr,
               //                               T_pint8, "", InsertPt)),
               cast<Value>(Constant::getIntegerValue(
                   T_size,
                   APInt(8 * sizeof(size_t),
                         cxxsizeofType(Cxx,
                                       (void *)PVD->getType().getTypePtr())))),
               cast<Value>(Constant::getIntegerValue(
                   T_prjlvalue,
                   APInt(8 * sizeof(void *), (uint64_t)juliatypes[i++])))},
              "", InsertPt);
          // Preserve this until the current insert point (we might do
          // additional allocations in the middle)
          Value *Tok = CallInst::Create(gc_preserve_begin_func, {Allocation},
                                        "", InsertPt);
          builder.CreateCall(gc_preserve_end_func, {Tok});
          Value *PFO =
              CallInst::Create(pointer_from_objref_func,
                               {CastInst::CreatePointerBitCastOrAddrSpaceCast(
                                   Allocation, T_pdjlvalue, "", InsertPt)},
                               "", InsertPt);
          llvm::Value *Replacement = CastInst::Create(
              Instruction::BitCast, PFO, ArgPtr->getType(), "", InsertPt);
          ArgPtr->replaceAllUsesWith(Replacement);
          CallArgs.push_back(Allocation);
        } else {
          bool isboxed;
          bool isVoid =
              f_julia_type_to_llvm(juliatypes[i++], &isboxed)->isVoidTy();
          ...
          if (specsig) {
            ...
            if (DestI->getType()->isPointerTy() &&
                !cast<PointerType>(ArgPtr->getType())
                     ->getElementType()
                     ->isPointerTy())
              CallArgs.push_back(
                  builder.CreateBitCast(ArgPtr, DestI->getType()));
            else
              CallArgs.push_back(builder.CreateLoad(builder.CreateBitCast(
                  ArgPtr, PointerType::get(DestI->getType(), 0))));
          } else {
            llvm::Value *Val =
                isVoid
                    ? ConstantPointerNull::get(cast<PointerType>(T_prjlvalue))
                    : builder.CreatePointerBitCastOrAddrSpaceCast(
                          builder.CreateLoad(ArgPtr), T_prjlvalue);
      ...
      if (specsig) {
        ...
        Call = builder.CreateCall(
            FTy, builder.CreateBitCast(F, FTy->getPointerTo()), CallArgs);
        if (firstIsEnv)
    ...
    if (isboxed ||
        (Call->getType()->isPointerTy() && cast<PointerType>(Call->getType())
                                               ->getElementType()
                                               ->isAggregateType())) {
      ...
      if (F->getReturnType() != FTy->getReturnType()) {
        ...
        builder.CreateStore(Call,
                            builder.CreateBitCast(
                                A, llvm::PointerType::get(Call->getType(), 0)));
    ...
    for (Function::const_arg_iterator I = F->arg_begin(), E = F->arg_end();
      ...
      if (DestI->getType() != I->getType()) {
        ...
        llvm::Value *A = builder.CreateAlloca(
            cast<llvm::PointerType>(I->getType())->getElementType());
        builder.CreateStore(
            &*DestI, builder.CreateBitCast(
                         A, llvm::PointerType::get(DestI->getType(), 0)));
        args.push_back(A);
      } else {
    ...
    if (F->getReturnType() != FTy->getReturnType()) {
      ...
      builder.CreateStore(
          Call,
          builder.CreateBitCast(A, llvm::PointerType::get(Call->getType(), 0)));
      Ret = builder.CreateLoad(A);
    }
...
JL_DLLEXPORT void *DeleteUnusedArguments(llvm::Function *F, uint64_t *dtodelete,
                                         size_t ntodelete) {
...
JL_DLLEXPORT void ReplaceFunctionForDecl(CxxInstance *Cxx,
                                         clang::FunctionDecl *D,
                                         llvm::Function *F, bool DoInline,
                                         bool specsig, bool firstIsEnv,
                                         bool *needsbox, void *retty,
                                         void **juliatypes, bool newgc) {
  const clang::CodeGen::CGFunctionInfo &FI =
      Cxx->CGM->getTypes().arrangeGlobalDeclaration(D);
  ...
  llvm::Function *NF =
      CloneFunctionAndAdjust(Cxx, F, Ty, true, &CCI, FI, D, specsig, firstIsEnv,
                             needsbox, retty, juliatypes, newgc);
  ...
  // OF->print(llvm::errs(), nullptr);
  // NF->print(llvm::errs(), nullptr);
...
JL_DLLEXPORT void *ActOnStartOfFunction(CxxInstance *Cxx,
                                        clang::FunctionDecl *D,
                                        bool ScopeIsNull = false) {
  clang::Sema &sema = Cxx->CI->getSema();
  // ContextRAII SavedContext(sema, DC);
  return (void *)sema.ActOnStartOfFunctionDef(
      ScopeIsNull ? nullptr : Cxx->Parser->getCurScope(), D);
}

JL_DLLEXPORT bool ParseFunctionStatementBody(CxxInstance *Cxx, clang::Decl *D) {
  clang::Parser::ParseScope BodyScope(Cxx->Parser, clang::Scope::FnScope |
  ...
  if (cast<clang::FunctionDecl>(D)->getReturnType()->isUndeducedType()) {
    ...
    if (last && isa<clang::Expr>(last)) {
      ...
      sema.DeduceFunctionTypeFromReturnExpr(cast<clang::FunctionDecl>(D),
                                            ReturnLoc, RetExpr,
                                            cast<clang::FunctionDecl>(D)
                                                ->getReturnType()
...
JL_DLLEXPORT void ActOnFinishNamespaceDef(CxxInstance *Cxx, clang::Decl *D) {
  ...
  Cxx->CI->getSema().ActOnFinishNamespaceDef(D, getTrivialSourceLocation(Cxx));
}

// For codegen.jl

JL_DLLEXPORT int typeconstruct(CxxInstance *Cxx, void *type,
                               clang::Expr **rawexprs, size_t nexprs,
                               void **ret) {
  ...
  if (Exprs.size() == 1) {
    ...
    Result = sema.BuildCXXFunctionalCastExpr(TInfo, Ty,
                                             getTrivialSourceLocation(Cxx), Arg,
                                             getTrivialSourceLocation(Cxx));
    if (Result.isInvalid())
      return false;
  ...
  if (sema.RequireNonAbstractType(
          getTrivialSourceLocation(Cxx), Ty,
          clang::diag::err_allocation_of_abstract_type)) {
    return false;
  }

  clang::InitializedEntity Entity =
      clang::InitializedEntity::InitializeTemporary(TInfo);
...
JL_DLLEXPORT void *BuildCXXNewExpr(CxxInstance *Cxx, clang::Type *type,
                                   clang::Expr **exprs, size_t nexprs) {
  clang::QualType Ty = clang::QualType::getFromOpaquePtr(type);
  clang::SourceManager &sm = Cxx->CI->getSourceManager();
  auto DIR = clang::SourceRange(sm.getLocForStartOfFile(sm.getMainFileID()),
                                sm.getLocForStartOfFile(sm.getMainFileID()));
  clang::ParenListExpr *initializer =
      clang::ParenListExpr::CreateEmpty(Cxx->CI->getASTContext(), 0);
  return cast<clang::CXXNewExpr>(
      Cxx->CI->getSema()
          .BuildCXXNew(clang::SourceRange(), false, clang::SourceLocation(),
                       clang::MultiExprArg(), clang::SourceLocation(),
                       clang::SourceRange(), Ty,
                       Cxx->CI->getASTContext().getTrivialTypeSourceInfo(Ty),
                       Optional<clang::Expr *>(llvm::None), DIR, initializer)
          .get());
}

JL_DLLEXPORT void *EmitCXXNewExpr(CxxInstance *Cxx, LLVMBuilderRef IRB,
                                  clang::Expr *E) {
  // sync builder states
  auto &CGBuilder = Cxx->CGF->Builder;
  llvm::IRBuilder<> *Builder = llvm::unwrap(IRB);
  CGBuilder.SetInsertPoint(Builder->GetInsertBlock(),
                           Builder->GetInsertPoint());

  assert(isa<clang::CXXNewExpr>(E));
  auto ret = Cxx->CGF->EmitCXXNewExpr(cast<clang::CXXNewExpr>(E));

  Builder->SetInsertPoint(CGBuilder.GetInsertBlock(),
                          CGBuilder.GetInsertPoint());
  CGBuilder.ClearInsertionPoint();

  return ret;
}

JL_DLLEXPORT void *build_call_to_member(CxxInstance *Cxx, clang::Expr *MemExprE,
                                        clang::Expr **exprs, size_t nexprs) {
  if (MemExprE->getType() == Cxx->CI->getASTContext().BoundMemberTy ||
      MemExprE->getType() == Cxx->CI->getASTContext().OverloadTy)
    return (void *)Cxx->CI->getSema()
        .BuildCallToMemberFunction(
            nullptr, MemExprE, getTrivialSourceLocation(Cxx),
            clang::MultiExprArg(exprs, nexprs), getTrivialSourceLocation(Cxx))
        .get();
...
JL_DLLEXPORT void *PerformMoveOrCopyInitialization(CxxInstance *Cxx, void *rt,
                                                   clang::Expr *expr) {
  clang::InitializedEntity Entity =
  ...
  return (void *)Cxx->CI->getSema()
      .PerformMoveOrCopyInitialization(
          Entity, nullptr, clang::QualType::getFromOpaquePtr(rt), expr, true)
      .get();
}

// For CxxREPL
JL_DLLEXPORT void *clang_compiler(CxxInstance *Cxx) { return (void *)Cxx->CI; }
...
public:
  ...
  void VisitDeclStmt(clang::DeclStmt *Node) {
    for (clang::DeclStmt::decl_iterator I = Node->decl_begin(),
                                        E = Node->decl_end();
         I != E; ++I)
      Visit(*I);
  }
  ...
  void VisitBlockExpr(clang::BlockExpr *Node) {
    // The BlockDecl is also visited by 'VisitDeclContext()'.  No need to visit
    // it twice.
  }

  void VisitStmt(clang::Stmt *Node) {
    for (clang::StmtIterator I = Node->children().begin();
...
public:
  ...
  virtual void HandleTagDeclDefinition(clang::TagDecl *D) {
    ...
    // point.
    if (Cxx.CI->getASTContext().getLangOpts().CPlusPlus &&
...
public:
  ...
  virtual bool LookupUnqualified(clang::LookupResult &R, clang::Scope *S) {
    if (R.getLookupName().getAsString() == "__julia" &&
        julia_namespace != nullptr) {
...
JL_DLLEXPORT void init_llvm_context(LLVMContextRef ctx) {
  jl_LLVMContext = llvm::unwrap(ctx);
}

JL_DLLEXPORT LLVMContextRef get_llvm_context(void) {
  return llvm::wrap(jl_LLVMContext);
}

JL_DLLEXPORT LLVMModuleRef get_llvm_module(CxxInstance *Cxx) {
  return llvm::wrap(Cxx->shadow);
}

JL_DLLEXPORT void init_types(LLVMTypeRef _T_pvalue_llvmt) {
  T_jlvalue =
      llvm::unwrap<llvm::PointerType>(_T_pvalue_llvmt)->getElementType();
  T_pjlvalue = llvm::PointerType::get(T_jlvalue, 0);
  T_prjlvalue = llvm::PointerType::get(T_jlvalue, AddressSpace::Tracked);
}

JL_DLLEXPORT void init_clang_instance(CxxInstance *Cxx, LLVMModuleRef mod,
                                      const char **cmd_args_with_src,
                                      int num_cmd_args) {
  ...
  Cxx->shadow = llvm::unwrap(mod);
  Cxx->CGM = new clang::CodeGen::CodeGenModule(
      Cxx->CI->getASTContext(), Cxx->CI->getHeaderSearchOpts(),
      Cxx->CI->getPreprocessorOpts(), Cxx->CI->getCodeGenOpts(), *Cxx->shadow,
      Cxx->CI->getDiagnostics());
  Cxx->CGF = new clang::CodeGen::CodeGenFunction(*Cxx->CGM);
  Cxx->CGF->CurFuncDecl = nullptr;
  Cxx->CGF->CurCodeDecl = nullptr;

  // Cxx isn't fully initialized yet, but that's fine since JuliaCodeGenerator
  ...
  const clang::FileEntry *MainFile =
      Cxx->CI->getFileManager().getVirtualFile("/Cxx.h", 0, time(0));
  sm.overrideFileContents(
      MainFile, llvm::WritableMemoryBuffer::getNewMemBuffer(0, "/Cxx.h"));
  ...
  clang::ExternalASTSource *External =
      Cxx->CI->getASTContext().getExternalSource();
  if (External)
...
JL_DLLEXPORT void *setup_cpp_env(CxxInstance *Cxx, LLVMValueRef jlfunc) {
  ...
  llvm::Function *w = llvm::unwrap<llvm::Function>(jlfunc);
  assert(w != nullptr);
  cur_func = w;
  ...
  Cxx->CGF->Builder.SetInsertPoint(bb0);
  // clang expects to alloca memory before the AllocaInsertPt
  // typically, clang would create this pointer when it started emitting the
  // function instead, we create a dummy reference here for efficiency, we avoid
  ...
  if (bb0->empty()) {
    ...
    Cxx->CGF->AllocaInsertPt = alloca_bb_ptr = new llvm::BitCastInst(
        Undef, llvm::Type::getInt32Ty(*jl_LLVMContext), "", bb0);
  } else {
    Cxx->CGF->AllocaInsertPt = &(bb0->front());
  }
...
JL_DLLEXPORT void cleanup_cpp_env(CxxInstance *Cxx, cppcall_state_t *state) {
  ...
  const clang::CodeGen::CGFunctionInfo &fnInfo =
      Cxx->CGM->getTypes().arrangeBuiltinFunctionCall(
          Cxx->CI->getASTContext().VoidTy, args);
  Cxx->CGF->CurFnInfo = &fnInfo;
  Cxx->CGF->FinishFunction();
  auto bk = Cxx->CGF->ReturnBlock.getBlock();
  assert(bk != nullptr);
  // bk->eraseFromParent();
  Cxx->CGF->ReturnBlock =
      Cxx->CGF->getJumpDestInCurrentScope(Cxx->CGF->createBasicBlock("return"));
  ...
  // for (auto I = jl_Module.global_begin(), E = jl_Module.global_end(); I != E;
  //      ++I) {
  //   if (I->hasPrivateLinkage()){
  //     I->print(llvm::errs(), false);
  //     I->setLinkage(llvm::GlobalValue::LinkageTypes::LinkOnceODRLinkage);
  //     I->print(llvm::errs(), false);
  //   }
...
*/
JL_DLLEXPORT void *CreateCallExpr(CxxInstance *Cxx, clang::Expr *Fn,
                                  clang::Expr **exprs, size_t nexprs) {
  return Cxx->CI->getSema()
      .ActOnCallExpr(nullptr, Fn, getTrivialSourceLocation(Cxx),
                     clang::MultiExprArg(exprs, nexprs),
                     getTrivialSourceLocation(Cxx))
      .get();
}

JL_DLLEXPORT void *CreateVarDecl(CxxInstance *Cxx, void *DC, char *name,
                                 void *type) {
  clang::QualType T = clang::QualType::getFromOpaquePtr(type);
  clang::VarDecl *D = clang::VarDecl::Create(
      Cxx->CI->getASTContext(), (clang::DeclContext *)DC,
      getTrivialSourceLocation(Cxx), getTrivialSourceLocation(Cxx),
      Cxx->CI->getPreprocessor().getIdentifierInfo(name), T,
      Cxx->CI->getASTContext().getTrivialTypeSourceInfo(T), clang::SC_Extern);
  return D;
}

JL_DLLEXPORT void *CreateFunctionDecl(CxxInstance *Cxx, void *DC, char *name,
                                      void *type, int isextern) {
  clang::QualType T = clang::QualType::getFromOpaquePtr(type);
  clang::FunctionDecl *D = clang::FunctionDecl::Create(
      Cxx->CI->getASTContext(), (clang::DeclContext *)DC,
      getTrivialSourceLocation(Cxx), getTrivialSourceLocation(Cxx),
      clang::DeclarationName(
          Cxx->CI->getPreprocessor().getIdentifierInfo(name)),
      T, Cxx->CI->getASTContext().getTrivialTypeSourceInfo(T),
      isextern ? clang::SC_Extern : clang::SC_None);

  // check if we are compiling for a device
  if (Cxx->CI->getLangOpts().CUDAIsDevice == 1) {
    D->addAttr(clang::CUDADeviceAttr::CreateImplicit(Cxx->CI->getASTContext(),
                                                     D->getLocation()));
  }
  return D;
}

JL_DLLEXPORT void *CreateCxxCallMethodDecl(CxxInstance *Cxx,
                                           clang::CXXRecordDecl *TheClass,
                                           void *QT) {
  clang::DeclarationName MethodName =
      Cxx->CI->getASTContext().DeclarationNames.getCXXOperatorName(
          clang::OO_Call);
  ...
  clang::CXXMethodDecl *Method = clang::CXXMethodDecl::Create(
      Cxx->CI->getASTContext(), TheClass, getTrivialSourceLocation(Cxx),
      clang::DeclarationNameInfo(MethodName, getTrivialSourceLocation(Cxx),
                                 MethodNameLoc),
...
JL_DLLEXPORT void *CreateParmVarDecl(CxxInstance *Cxx, void *type, char *name,
                                     int used) {
  clang::QualType T = clang::QualType::getFromOpaquePtr(type);
  clang::ParmVarDecl *d = clang::ParmVarDecl::Create(
      Cxx->CI->getASTContext(),
      Cxx->CI->getASTContext()
          .getTranslationUnitDecl(), // This is wrong, hopefully it doesn't
                                     // matter
      getTrivialSourceLocation(Cxx), getTrivialSourceLocation(Cxx),
      &Cxx->CI->getPreprocessor().getIdentifierTable().getOwn(name), T,
      Cxx->CI->getASTContext().getTrivialTypeSourceInfo(T), clang::SC_None,
      nullptr);
  if (used)
    d->setIsUsed();
  return (void *)d;
...
JL_DLLEXPORT void *CreateTypeDefDecl(CxxInstance *Cxx, clang::DeclContext *DC,
                                     char *name, void *type) {
  clang::QualType T = clang::QualType::getFromOpaquePtr(type);
  return (void *)clang::TypedefDecl::Create(
      Cxx->CI->getASTContext(), DC, getTrivialSourceLocation(Cxx),
      getTrivialSourceLocation(Cxx),
      &Cxx->CI->getPreprocessor().getIdentifierTable().getOwn(name),
      Cxx->CI->getASTContext().getTrivialTypeSourceInfo(T));
}

JL_DLLEXPORT void SetFDParams(clang::FunctionDecl *FD,
                              clang::ParmVarDecl **PVDs, size_t npvds) {
...
JL_DLLEXPORT void AssociateValue(CxxInstance *Cxx, LLVMBuilderRef IRB,
                                 clang::Decl *d, void *type, LLVMValueRef val) {
  ...
  Cxx->CGF->EmitParmDecl(*vd,
                         clang::CodeGen::CodeGenFunction::ParamValue::forDirect(
                             Cxx->CGF->Builder.CreateBitCast(V, Ty)),
                         0);
...
JL_DLLEXPORT void *CreateDeclRefExpr(CxxInstance *Cxx, clang::ValueDecl *D,
                                     clang::CXXScopeSpec *scope, int islvalue) {
  clang::QualType T = D->getType();
  return (void *)clang::DeclRefExpr::Create(
      Cxx->CI->getASTContext(),
      scope ? scope->getWithLocInContext(Cxx->CI->getASTContext())
            : clang::NestedNameSpecifierLoc(nullptr, nullptr),
...
JL_DLLEXPORT void *emitcallexpr(CxxInstance *Cxx, LLVMBuilderRef IRB,
                                clang::Expr *E, LLVMValueRef RSlot) {
  if (isa<clang::CXXBindTemporaryExpr>(E))
    E = cast<clang::CXXBindTemporaryExpr>(E)->getSubExpr();
  llvm::Value *rslot = llvm::unwrap(RSlot);
  ...
  clang::CodeGen::RValue ret = Cxx->CGF->EmitCallExpr(
      CE, clang::CodeGen::ReturnValueSlot(
...
JL_DLLEXPORT void emitexprtomem(CxxInstance *Cxx, LLVMBuilderRef IRB,
                                clang::Expr *E, llvm::Value *addr, int isInit) {
  // sync builder states
  auto &CGBuilder = Cxx->CGF->Builder;
  llvm::IRBuilder<> *Builder = llvm::unwrap(IRB);
  CGBuilder.SetInsertPoint(Builder->GetInsertBlock(),
  ...
  Cxx->CGF->EmitAnyExprToMem(
      E, clang::CodeGen::Address(addr, clang::CharUnits::One()),
...
JL_DLLEXPORT void *EmitAnyExpr(CxxInstance *Cxx, LLVMBuilderRef IRB,
                               clang::Expr *E, llvm::Value *rslot) {
  // sync builder states
  auto &CGBuilder = Cxx->CGF->Builder;
  llvm::IRBuilder<> *Builder = llvm::unwrap(IRB);
  CGBuilder.SetInsertPoint(Builder->GetInsertBlock(),
...
JL_DLLEXPORT void *decl_context(clang::Decl *decl) {
  if (isa<clang::TypedefNameDecl>(decl)) {
    decl = dyn_cast<clang::TypedefNameDecl>(decl)
               ->getUnderlyingType()
               .getTypePtr()
               ->getAsCXXRecordDecl();
  }
  ...
  {
      auto ptr =
  cast<clang::ClassTemplateSpecializationDecl>(decl)->getSpecializedTemplateOrPartial();
...
JL_DLLEXPORT void *
getTemplateArgs(clang::ClassTemplateSpecializationDecl *tmplt) {
...
JL_DLLEXPORT void *getTargTypeAtIdx(clang::TemplateArgumentList *targs,
                                    size_t i) {
  return getTargType(&targs->get(i));
}

JL_DLLEXPORT void *getTSTTargTypeAtIdx(clang::TemplateSpecializationType *targs,
                                       size_t i) {
...
JL_DLLEXPORT void *getTargIntegralTypeAtIdx(clang::TemplateArgumentList *targs,
                                            size_t i) {
...
JL_DLLEXPORT int getTargKindAtIdx(clang::TemplateArgumentList *targs,
                                  size_t i) {
  return getTargKind(&targs->get(i));
}

JL_DLLEXPORT int getTSTTargKindAtIdx(clang::TemplateSpecializationType *TST,
                                     size_t i) {
  return getTargKind(&TST->getArg(i));
}

JL_DLLEXPORT size_t getTargPackAtIdxSize(clang::TemplateArgumentList *targs,
                                         size_t i) {
  return targs->get(i).getPackAsArray().size();
}

JL_DLLEXPORT void *getTargPackAtIdxTargAtIdx(clang::TemplateArgumentList *targs,
                                             size_t i, size_t j) {
  return (void *)&targs->get(i).getPackAsArray()[j];
}

JL_DLLEXPORT int64_t getTargAsIntegralAtIdx(clang::TemplateArgumentList *targs,
                                            size_t i) {
...
JL_DLLEXPORT void *getReferenceTo(CxxInstance *Cxx, void *T) {
  return Cxx->CI->getASTContext()
      .getLValueReferenceType(clang::QualType::getFromOpaquePtr(T))
      .getAsOpaquePtr();
}
...
JL_DLLEXPORT void *getIncompleteArrayType(CxxInstance *Cxx, void *T) {
  return Cxx->CI->getASTContext()
      .getIncompleteArrayType(clang::QualType::getFromOpaquePtr(T),
...
JL_DLLEXPORT void *createDerefExpr(CxxInstance *Cxx, clang::Expr *expr) {
  return (void *)Cxx->CI->getSema()
      .CreateBuiltinUnaryOp(getTrivialSourceLocation(Cxx), clang::UO_Deref,
                            expr)
      .get();
}
...
JL_DLLEXPORT void *createAddrOfExpr(CxxInstance *Cxx, clang::Expr *expr) {
  return (void *)Cxx->CI->getSema()
      .CreateBuiltinUnaryOp(getTrivialSourceLocation(Cxx), clang::UO_AddrOf,
                            expr)
      .get();
}
...
JL_DLLEXPORT void *CreateBinOp(CxxInstance *Cxx, clang::Scope *S, int opc,
  ...
  return (void *)Cxx->CI->getSema()
      .BuildBinOp(S, clang::SourceLocation(), (clang::BinaryOperatorKind)opc,
                  LHS, RHS)
      .get();
}

JL_DLLEXPORT void *BuildMemberReference(CxxInstance *Cxx, clang::Expr *base,
                                        clang::Type *t, int IsArrow,
                                        char *name) {
  clang::DeclarationName DName(&Cxx->CI->getASTContext().Idents.get(name));
  clang::Sema &sema = Cxx->CI->getSema();
  ...
  return (void *)sema
      .BuildMemberReferenceExpr(
          base, base->getType(), getTrivialSourceLocation(Cxx), IsArrow, scope,
          clang::SourceLocation(), nullptr,
          clang::DeclarationNameInfo(DName, getTrivialSourceLocation(Cxx)),
          nullptr, nullptr)
      .get();
}

JL_DLLEXPORT void *BuildDeclarationNameExpr(CxxInstance *Cxx, char *name,
                                            clang::DeclContext *ctx) {
  clang::Sema &sema = Cxx->CI->getSema();
  clang::SourceManager &sm = Cxx->CI->getSourceManager();
  clang::CXXScopeSpec spec;
  ...
  clang::LookupResult R(sema, DName, getTrivialSourceLocation(Cxx),
                        clang::Sema::LookupAnyName);
  sema.LookupQualifiedName(R, ctx, false);
  return (void *)sema.BuildDeclarationNameExpr(spec, R, false).get();
}

JL_DLLEXPORT void sync_builder_states(CxxInstance *Cxx, LLVMBuilderRef IRB) {
  auto &CGBuilder = Cxx->CGF->Builder;
  llvm::IRBuilder<> *Builder = llvm::unwrap(IRB);
  Builder->SetInsertPoint(CGBuilder.GetInsertBlock(),
                          CGBuilder.GetInsertPoint());
  Builder->SetCurrentDebugLocation(CGBuilder.getCurrentDebugLocation());
  CGBuilder.ClearInsertionPoint();
}

JL_DLLEXPORT void cdump(void *decl) { ((clang::Decl *)decl)->dump(); }

JL_DLLEXPORT void dcdump(clang::DeclContext *DC) { DC->dumpDeclContext(); }

JL_DLLEXPORT void exprdump(void *expr) { ((clang::Expr *)expr)->dump(); }

JL_DLLEXPORT void typedump(void *t) { ((clang::Type *)t)->dump(); }
...
#define W(M, ARG)                                                              \
  JL_DLLEXPORT void *M(ARG *p) { return (void *)p->M(); }

W(getPointeeCXXRecordDecl, clang::Type)
W(getAsCXXRecordDecl, clang::Type)

#define ISAD(NS, T, ARGT)                                                      \
  JL_DLLEXPORT int isa##T(ARGT *p) { return llvm::isa<NS::T>(p); }             \
  JL_DLLEXPORT void *dcast##T(ARGT *p) { return llvm::dyn_cast<NS::T>(p); }
...
JL_DLLEXPORT size_t cxxsizeof(CxxInstance *Cxx, clang::CXXRecordDecl *decl) {
  ...
  Cxx->CI->getSema().RequireCompleteType(
      getTrivialSourceLocation(Cxx), clang::QualType(decl->getTypeForDecl(), 0),
      0);
...
JL_DLLEXPORT size_t cxxsizeofType(CxxInstance *Cxx, void *t) {
  ...
  return dl.getTypeSizeInBits(
             cgt->ConvertTypeForMem(clang::QualType::getFromOpaquePtr(t))) /
         8;
}

JL_DLLEXPORT LLVMTypeRef ConvertTypeForMem(CxxInstance *Cxx, void *t) {
  return llvm::wrap(Cxx->CGM->getTypes().ConvertTypeForMem(
...
JL_DLLEXPORT void deleteCXXScopeSpec(clang::CXXScopeSpec *spec) { delete spec; }

JL_DLLEXPORT void ExtendNNS(CxxInstance *Cxx,
                            clang::NestedNameSpecifierLocBuilder *builder,
                            clang::NamespaceDecl *d) {
  builder->Extend(Cxx->CI->getASTContext(), d, getTrivialSourceLocation(Cxx),
                  getTrivialSourceLocation(Cxx));
}

JL_DLLEXPORT void
ExtendNNSIdentifier(CxxInstance *Cxx,
                    clang::NestedNameSpecifierLocBuilder *builder,
                    const char *Name) {
  clang::Preprocessor &PP = Cxx->Parser->getPreprocessor();
  // Get the identifier.
  clang::IdentifierInfo *Id = PP.getIdentifierInfo(Name);
  builder->Extend(Cxx->CI->getASTContext(), Id, getTrivialSourceLocation(Cxx),
                  getTrivialSourceLocation(Cxx));
}

JL_DLLEXPORT void ExtendNNSType(CxxInstance *Cxx,
                                clang::NestedNameSpecifierLocBuilder *builder,
                                void *t) {
  ...
  builder->Extend(Cxx->CI->getASTContext(), clang::SourceLocation(),
                  TLBuilder.getTypeLocInContext(Cxx->CI->getASTContext(), T),
                  getTrivialSourceLocation(Cxx));
}

JL_DLLEXPORT void *makeFunctionType(CxxInstance *Cxx, void *rt, void **argts,
                                    size_t nargs) {
  clang::QualType T;
  if (rt == nullptr) {
    T = Cxx->CI->getASTContext().getAutoType(
        clang::QualType(), clang::AutoTypeKeyword::DecltypeAuto,
        /*IsDependent*/ false);
  } else {
    T = clang::QualType::getFromOpaquePtr(rt);
  }
  clang::QualType *qargs =
      (clang::QualType *)__builtin_alloca(nargs * sizeof(clang::QualType));
  for (size_t i = 0; i < nargs; ++i)
    qargs[i] = clang::QualType::getFromOpaquePtr(argts[i]);
  clang::FunctionProtoType::ExtProtoInfo EPI;
  return Cxx->CI->getASTContext()
      .getFunctionType(T, llvm::ArrayRef<clang::QualType>(qargs, nargs), EPI)
      .getAsOpaquePtr();
}

JL_DLLEXPORT void *makeMemberFunctionType(CxxInstance *Cxx, void *FT,
                                          clang::Type *cls) {
  return Cxx->CI->getASTContext()
      .getMemberPointerType(clang::QualType::getFromOpaquePtr(FT), cls)
      .getAsOpaquePtr();
}
...
JL_DLLEXPORT void *getMemberPointerPointee(clang::Type *mptr) {
  return cast<clang::MemberPointerType>(mptr)
      ->getPointeeType()
      .getAsOpaquePtr();
}
...
JL_DLLEXPORT void MarkDeclarationsReferencedInExpr(CxxInstance *Cxx,
                                                   clang::Expr *e) {
  clang::Sema &sema = Cxx->CI->getSema();
...
JL_DLLEXPORT int isDeclInvalid(clang::Decl *D) { return D->isInvalidDecl(); }
...
JL_DLLEXPORT int RequireCompleteType(CxxInstance *Cxx, clang::Type *t) {
  clang::Sema &sema = Cxx->CI->getSema();
  return sema.RequireCompleteType(getTrivialSourceLocation(Cxx),
                                  clang::QualType(t, 0), 0);
}

JL_DLLEXPORT void *CreateTemplatedFunction(CxxInstance *Cxx, char *Name,
  ...
  clang::Sema &sema = Cxx->CI->getSema();
  return sema.ActOnTemplateDeclarator(
      nullptr, clang::MultiTemplateParamsArg(args, nargs), D);
}

JL_DLLEXPORT void *ActOnTypeParameter(CxxInstance *Cxx, char *Name,
                                      unsigned Position) {
  clang::Sema &sema = Cxx->CI->getSema();
  clang::ParsedType DefaultArg;
  clang::Preprocessor &PP = Cxx->Parser->getPreprocessor();
  clang::Scope S(nullptr, clang::Scope::TemplateParamScope,
                 Cxx->CI->getDiagnostics());
...
JL_DLLEXPORT void *CreateAnonymousClass(CxxInstance *Cxx, clang::Decl *Scope) {
  clang::CXXRecordDecl *RD = clang::CXXRecordDecl::Create(
      ...
      clang::dyn_cast<clang::DeclContext>(Scope), clang::SourceLocation(),
      clang::SourceLocation(), nullptr);
  RD->setImplicit(true);
  RD->startDefinition();
  return (void *)RD;
...
JL_DLLEXPORT void *AddCallOpToClass(clang::CXXRecordDecl *TheClass,
                                    clang::CXXMethodDecl *Method) {
  TheClass->addDecl(Method);
  return Method;
}

JL_DLLEXPORT void FinalizeAnonClass(CxxInstance *Cxx,
                                    clang::CXXRecordDecl *TheClass) {
...
JL_DLLEXPORT void *ActOnTypeParameterParserScope(CxxInstance *Cxx, char *Name,
                                                 unsigned Position) {
  clang::Sema &sema = Cxx->CI->getSema();
  clang::ParsedType DefaultArg;
  clang::Preprocessor &PP = Cxx->Parser->getPreprocessor();
...
JL_DLLEXPORT void *
CreateTemplateParameterList(CxxInstance *Cxx, clang::NamedDecl **D, size_t ND) {
  return (void *)clang::TemplateParameterList::Create(
      Cxx->CI->getASTContext(), clang::SourceLocation(),
      clang::SourceLocation(), ArrayRef<clang::NamedDecl *>(D, ND),
      clang::SourceLocation(), nullptr);
}

JL_DLLEXPORT void *
CreateFunctionTemplateDecl(CxxInstance *Cxx, clang::DeclContext *DC,
...
JL_DLLEXPORT void *newFDVector() {
  return (void *)new std::vector<clang::FunctionDecl *>;
}
JL_DLLEXPORT void deleteFDVector(void *v) {
  delete ((std::vector<clang::FunctionDecl *> *)v);
}
JL_DLLEXPORT void copyFDVector(void *to,
                               std::vector<clang::FunctionDecl *> *from) {
  memcpy(to, from->data(), sizeof(clang::FunctionDecl *) * from->size());
}
JL_DLLEXPORT size_t getFDVectorSize(std::vector<clang::FunctionDecl *> *v) {
  return v->size();
}

JL_DLLEXPORT void
getSpecializations(clang::FunctionTemplateDecl *FTD,
                   std::vector<clang::FunctionDecl *> *specs) {
  for (auto it : FTD->specializations())
    specs->push_back(it);
}

JL_DLLEXPORT const char *getMangledFunctionName(CxxInstance *Cxx,
                                                clang::FunctionDecl *D) {
  return Cxx->CGM->getMangledName(clang::GlobalDecl(D)).data();
}

JL_DLLEXPORT const void *
getTemplateSpecializationArgs(clang::FunctionDecl *FD) {
...
static clang::ExprResult CreateFunctionRefExpr(
    clang::Sema &S, clang::FunctionDecl *Fn, clang::NamedDecl *FoundDecl,
...
JL_DLLEXPORT void *CreateCStyleCast(CxxInstance *Cxx, clang::Expr *E,
                                    clang::Type *T) {
...
JL_DLLEXPORT void *CreateThisExpr(CxxInstance *Cxx, void *T) {
  return (void *)new (Cxx->CI->getASTContext()) clang::CXXThisExpr(
      clang::SourceLocation(), clang::QualType::getFromOpaquePtr(T), false);
}

JL_DLLEXPORT void *CreateFunctionRefExprFD(CxxInstance *Cxx,
                                           clang::FunctionDecl *FD) {
  return CreateFunctionRefExpr(Cxx->CI->getSema(), FD, FD, false).get();
}

JL_DLLEXPORT void *
CreateFunctionRefExprFDTemplate(CxxInstance *Cxx,
                                clang::FunctionTemplateDecl *FTD) {
  return CreateFunctionRefExpr(Cxx->CI->getSema(), FTD->getTemplatedDecl(), FTD,
                               false)
      .get();
}
...
JL_DLLEXPORT void ActOnFinishFunctionBody(CxxInstance *Cxx,
                                          clang::FunctionDecl *FD,
                                          clang::Stmt *Body) {
  Cxx->CI->getSema().ActOnFinishFunctionBody(FD, Body, true);
}

JL_DLLEXPORT void *CreateLinkageSpec(CxxInstance *Cxx, clang::DeclContext *DC,
                                     unsigned kind) {
  return (void *)(clang::DeclContext *)clang::LinkageSpecDecl::Create(
      Cxx->CI->getASTContext(), DC, clang::SourceLocation(),
      clang::SourceLocation(), (clang::LinkageSpecDecl::LanguageIDs)kind, true);
}
...
JL_DLLEXPORT int IsDeclUsed(clang::Decl *D) { return D->isUsed(); }

JL_DLLEXPORT void emitDestroyCXXObject(CxxInstance *Cxx, llvm::Value *x,
                                       clang::Type *T) {
  clang::CXXRecordDecl *RD = T->getAsCXXRecordDecl();
  clang::CXXDestructorDecl *Destructor =
      Cxx->CI->getSema().LookupDestructor(RD);
  Cxx->CI->getSema().MarkFunctionReferenced(clang::SourceLocation(),
                                            Destructor);
  Cxx->CI->getSema().DefineUsedVTables();
  Cxx->CI->getSema().PerformPendingInstantiations(false);
  Cxx->CGF->destroyCXXObject(
      *Cxx->CGF, clang::CodeGen::Address(x, clang::CharUnits::One()),
      clang::QualType(T, 0));
}

JL_DLLEXPORT bool hasTrivialDestructor(CxxInstance *Cxx,
...
JL_DLLEXPORT int hasFDBody(clang::FunctionDecl *FD) { return FD->hasBody(); }

JL_DLLEXPORT void *
getUnderlyingTemplateDecl(clang::TemplateSpecializationType *TST) {
  return (void *)TST->getTemplateName().getAsTemplateDecl();
}

JL_DLLEXPORT void *
getOrCreateTemplateSpecialization(CxxInstance *Cxx,
                                  clang::FunctionTemplateDecl *FTD, void **T,
                                  size_t nargs) {
...
JL_DLLEXPORT void *CreateIntegerLiteral(CxxInstance *Cxx, uint64_t val,
                                        void *T) {
  clang::QualType QT = clang::QualType::getFromOpaquePtr(T);
  return (void *)clang::IntegerLiteral::Create(
      Cxx->CI->getASTContext(), llvm::APInt(8 * sizeof(uint64_t), val), QT,
      clang::SourceLocation());
}
...
JL_DLLEXPORT void *getUnderlyingTypeOfEnum(clang::Type *T) {
  return clang::cast<clang::EnumType>(T)
      ->getDecl()
      ->getIntegerType()
      .getAsOpaquePtr();
}
...
JL_DLLEXPORT void SetConstexpr(clang::VarDecl *VD) { VD->setConstexpr(true); }
...
JL_DLLEXPORT void *getTypeName(CxxInstance *Cxx, void *Ty) {
  ...
  Cxx->CGM->getCXXABI().getMangleContext().mangleCXXRTTIName(
      clang::QualType::getFromOpaquePtr(Ty), Out);
...
#include "unwind.h"
void __attribute__((noreturn)) (*process_cxx_exception)(
    uint64_t exceptionClass, _Unwind_Exception *unwind_exception);

_Unwind_Reason_Code __cxxjl_personality_v0(int version, _Unwind_Action actions,
